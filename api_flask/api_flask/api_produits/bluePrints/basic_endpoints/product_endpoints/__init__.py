...
def getProducts():
    ...
    return jsonify(produits = [product.to_dict() for product in products]), 200


@productBluePrint.route('/product', methods=['POST'])
def createProduct():
    request_data = json.loads(request.get_data())
    if all(value in request_data for value in ['code', 'nom', 'stock']):
        ...
        if not product:
            try:
                ...
                product = Produit(code, name, stock)
